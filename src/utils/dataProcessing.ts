...
import { ImportedData, FieldConfig, ValidationRule, ValidationError, DataRow } from '@/types';
...
export const generateAutoMapping = (
  ...
  importedHeaders.forEach(header => {
    ...
    fields.forEach(field => {
      const confidence = calculateSimilarity(header.toLowerCase(), field.label.toLowerCase()) ||
  ...
  return importedData.rows.map((row, index) => {
    ...
    Object.entries(mapping).forEach(([sourceColumn, targetField]) => {
      if (targetField && row[sourceColumn] !== undefined) {
        ...
        if (field) {
          const transformedValue = transformValue(row[sourceColumn], field.type);
