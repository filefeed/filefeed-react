  ...
  devtools(
    (set, get) => ({
      ...
      updateMapping: (sourceColumn, targetField) => {
        const state = get();
        const newMapping = { ...state.mappingState, [sourceColumn]: targetField };
      ...
      updateRowData: (rowId, fieldKey, value) => {
        ...
        const updatedData = state.processedData.map(row => {
          if (row.id === rowId) {
            ...
            if (field) {
              const rowIndex = state.processedData.findIndex(r => r.id === rowId);
      ...
      deleteRow: (rowId) => {
        const state = get();
        const updatedData = state.processedData.filter(row => row.id !== rowId);
        set({ processedData: updatedData });
